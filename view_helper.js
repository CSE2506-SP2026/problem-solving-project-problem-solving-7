...
function define_grouped_permission_checkboxes(id_prefix, which_groups = null) {
  ...
  let group_table = $(`
    <table id="${id_prefix}" class="ui-widget-content" width="100%">
        ...
        <tr id="${id_prefix}_deny_warning_row">
            <td colspan="3" id="${id_prefix}_deny_warning" style="font-size:11px; color:#b94a48; padding:2px 4px; display:none;">
                ⚠ Deny overrides all other permissions, including group and inherited allows.
            </td>
        </tr>
    </table>
    `);
  ...
  const perm_descriptions = {
    ...
    Read_Execute: "Read_Execute (view and run files)",
    Modify: "Modify (edit and delete)",
    Full_control: "Full_control (all permissions)",
    Special_permissions: "Special_permissions (custom settings)",
  };
  for (let g of which_groups) {
    let display_name = "";
    if (perm_descriptions[g]) {
      display_name = perm_descriptions[g];
    } else {
      display_name = g;
    }
    let row = $(`<tr id="${id_prefix}_row_${g}">
            <td id="${id_prefix}_${g}_name">${display_name}</td>
            </tr>`);
  ...
  group_table.find(".groupcheckbox").change(function () {
    ...
    let changed_row = group_table.find("#" + id_prefix + "_row_" + group);
    changed_row.addClass("checkbox-flash");
    window.setTimeout(function () {
      changed_row.removeClass("checkbox-flash");
    }, 800);
    let action = "";
    if (ptype == "deny") {
      if (checked) {
    ...
    let any_deny_checked = false;
    group_table.find('.groupcheckbox[ptype="deny"]').each(function () {
      if ($(this).prop("checked")) {
        any_deny_checked = true;
      }
    });
    if (any_deny_checked) {
      group_table.find(`#${id_prefix}_deny_warning`).show();
    } else {
      group_table.find(`#${id_prefix}_deny_warning`).hide();
    }
    $("#permdialog_last_action").text(
      "Last action: " + action + " " + group + " for " + username,
    );
  });
