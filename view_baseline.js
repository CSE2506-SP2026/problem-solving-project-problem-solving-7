    ...
    buttons: {
      Reset: {
        ...
        click: function () {
          ...
          let current_username = grouped_permissions.attr("username");
          if (acl_snapshot !== null) {
            path_to_file[filepath].acl = JSON.parse(
              JSON.stringify(acl_snapshot),
            );
            emitState();
            perm_dialog.attr("reselect_user", current_username);
            perm_dialog.attr("filepath", filepath);

            $("#permdialog_last_action").text("Changes reset to default.");
          }
        },
      },
      OK: {
      ...
      Advanced: {
        text: "Advanced (Inheritance & Special Permissions)",
...
perm_dialog.append(last_action_bar);
...
define_attribute_observer(perm_dialog, "filepath", function () {
  ...
  let reselect_username = perm_dialog.attr("reselect_user") || "";
  acl_snapshot = JSON.parse(JSON.stringify(path_to_file[current_filepath].acl));
  ...
  if (reselect_username && reselect_username.length > 0) {
    let user_elem = file_permission_users.find(
      `#permdialog_file_user_${reselect_username}`,
    );
    user_elem.addClass("ui-selected").siblings().removeClass("ui-selected");
    file_permission_users.attr("selected_item", reselect_username);
    grouped_permissions.attr("username", reselect_username);
    perm_dialog.attr("reselect_user", "");
  }
});
